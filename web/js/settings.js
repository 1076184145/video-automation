...
const editableGroups = [
  ...
  },
  {
    ...
    fields: [
      { env: "LLM_PROVIDER", path: ["optional_modules", "llm_provider"], type: "select", options: ["local", "openai", "openai-compatible", "google"] },
      { env: "LLM_MODEL", path: ["optional_modules", "llm_model"], placeholder: "settings.placeholder.llm_model" },
      { env: "LOCAL_MODELS_DIR", path: ["optional_modules", "local_models_dir"] },
      { env: "LOCAL_LLM_SERVER_PATH", path: ["optional_modules", "local_llm_server_path"] },
      { env: "LOCAL_LLM_MODEL_PATH", path: ["optional_modules", "local_llm_model_path"] },
      { env: "LOCAL_LLM_BASE_URL", path: ["optional_modules", "local_llm_base_url"] },
      { env: "LOCAL_LLM_CONTEXT_SIZE", path: ["optional_modules", "local_llm_context_size"], type: "number", min: 4096, step: 1024 },
      { env: "LOCAL_LLM_GPU_LAYERS", path: ["optional_modules", "local_llm_gpu_layers"], type: "number", min: 0, step: 1 },
      { env: "LOCAL_LLM_THREADS", path: ["optional_modules", "local_llm_threads"], type: "number", min: 1, step: 1 },
      { env: "LOCAL_LLM_STARTUP_TIMEOUT_SECONDS", path: ["optional_modules", "local_llm_startup_timeout_seconds"], type: "number", min: 1, step: 1 },
      { env: "LOCAL_LLM_REQUEST_TIMEOUT_SECONDS", path: ["optional_modules", "local_llm_request_timeout_seconds"], type: "number", min: 1, step: 1 },
      { env: "COVER_PROVIDER", path: ["covers", "provider"], type: "select", options: ["local", "openai", "openai-compatible", "openrouter", "google", "disabled"] },
      { env: "COVER_BASE_URL", path: ["covers", "base_url"] },
      { env: "COVER_MODEL", path: ["covers", "model"] },
      { env: "LOCAL_COVER_MODEL_PATH", path: ["covers", "local_model_path"] },
      { env: "LOCAL_COVER_DEVICE", path: ["covers", "local_device"] },
      { env: "LOCAL_COVER_QUANTIZATION", path: ["covers", "local_quantization"], type: "select", options: ["nf4", "int8", "none"] },
      { env: "LOCAL_COVER_MAX_SIDE", path: ["covers", "local_max_side"], type: "number", min: 512, step: 64 },
      { env: "LOCAL_COVER_STEPS", path: ["covers", "local_steps"], type: "number", min: 1, step: 1 },
      { env: "LOCAL_COVER_GUIDANCE_SCALE", path: ["covers", "local_guidance_scale"], type: "number", min: 0, step: 0.1 },
      { env: "LOCAL_COVER_SEED", path: ["covers", "local_seed"], type: "number", min: 0, step: 1 },
      { env: "LOCAL_COVER_MAX_SEQUENCE_LENGTH", path: ["covers", "local_max_sequence_length"], type: "number", min: 64, step: 64 },
