...
def evaluate_quality_gate(job_dir: Path | str, policy: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    max_chars = max(1, int(policy.get("subtitle_max_chars_per_line") or 18))
    ...
    for index, segment in enumerate(segments):
        ...
        if text and math.ceil(len(text) / max_chars) > max_lines:
            overflowing.append(index)
    if overflowing:
        blocking.append(_item(
            "subtitle_overflow",
            "One or more subtitles exceed the configured line limit.",
            segment_indexes=overflowing[:50],
            count=len(overflowing),
        ))
    elif segments:
        passed.append(_item("subtitles_fit", "Subtitle lines fit the configured limit."))
